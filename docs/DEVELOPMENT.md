...
```bash
REGISTRY=local TAG=0.1.4 ./deploy/build-images.sh
```

构建结果：

```text
registry.example.com/monitoring/inference-monitor-center:0.1.4
registry.example.com/monitoring/inference-monitor-node-musa:0.1.4
```

## 推送镜像

```bash
docker push registry.example.com/monitoring/inference-monitor-center:0.1.4
docker push registry.example.com/monitoring/inference-monitor-node-musa:0.1.4
```
